...
public class ConnectionPlay {
    ...
    private final String URL = "jdbc:mysql://localhost:3309/geektime";
    private final String NAME = "root";
    private final String PASSWORD = "TNB7ymq";
    ...
    private void insert() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            String sql = "insert into t_user(name, password, mobile) values ('" + randomString() + "', 'pwd', '123456789')";
            boolean execute = stmt.execute(sql);
            System.out.println(execute);
        }

        String sql = "insert into product_order(user_id, sku_id, count, actual_price, status) values (?,?,?, ?, ?)";
...
//                System.out.println("hikari get" + id + " " + name);
    ...
    public void runWithJDBCSpeedTest() throws Exception {
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            Statement stmt = conn.createStatement();
            ResultSet resultSet = stmt.executeQuery("select * from product_order");
//            System.out.println(resultSet);
            while (resultSet.next()) {
